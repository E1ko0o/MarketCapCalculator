...
class MousewheelSupport(object):
    ...
    def __init__(self, root_inner, horizontal_factor=2, vertical_factor=2):
        ...
        if OS == "Linux":
            root_inner.bind_all('<4>', self._on_mousewheel, add='+')
            root_inner.bind_all('<5>', self._on_mousewheel, add='+')
        else:
            # Windows and MacOS
            root_inner.bind_all("<MouseWheel>", self._on_mousewheel, add='+')
    ...
    def add_support_to(self, widget=None, xscrollbar=None, yscrollbar=None, what="units"):
...
class ScrollingArea(Frame, object):
    ...
    def __init__(self, master, width=None, anchor=N, height=None, scroll_horizontally=True,
...
class DataCell(Cell):
    def __init__(self, master, variable, width=500, anchor=W, border_color=None, border_width=1, padx=0, pady=0, background=None,
                 foreground=None, font=None):
        Cell.__init__(self, master, background=background, highlightbackground=border_color,
                      highlightcolor=border_color, highlightthickness=border_width, bd=0)

        self._message_widget = Message(self, textvariable=variable, font=font, background=background,
                                       foreground=foreground, width=width)
        self._message_widget.pack(expand=True, padx=padx, pady=pady, anchor=anchor)


class HeaderCell(Cell):
    def __init__(self, master, text, width=20, border_color=None, border_width=1, padx=0, pady=0, background=None,
                 foreground=None, font=None, anchor=CENTER, separator=True):
        Cell.__init__(self, master, background=background, highlightbackground=border_color,
                      highlightcolor=border_color, highlightthickness=border_width, bd=0)
        self.pack_propagate(False)

        self._header_label = Label(self, text=text, background=background,
                                   foreground=foreground, font=font, width=width)
        self._header_label.pack(padx=padx, pady=pady, expand=True)

        if separator and border_color is not None:
            separator = Frame(self, height=2, background=border_color, bd=0, highlightthickness=0, class_="Separator")
...
class Table(Frame):
    def __init__(self, master, column_headers: list, column_weights=None, column_min_widths=None, height=500,
                 padx=5, pady=5, cell_font=None, cell_foreground="black", cell_background="white",
                 cell_anchor=W, header_font=None, header_background="white", header_foreground="black",
                 header_anchor=CENTER, border_color="#999999", inner_border=True, outer_border=True,
                 stripped_rows=("#EEEEEE", "white"), on_change_data=None, scroll_horizontally=False,
                 scroll_vertically=True):
        outer_border_width = 1 if outer_border else 0
        ...
        self._innerborder_width = 1 if inner_border else 0

        self._data_vars = []

        self._columns = column_headers

        self._number_of_rows = 0
        self._number_of_columns = len(column_headers)

        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=1)

        self._head = Frame(self, highlightbackground=border_color, highlightcolor=border_color,
                           highlightthickness=outer_border_width, bd=0)
        self._head.grid(row=0, column=0, sticky=E + W)

        header_separator = False if outer_border else True

        for j in range(len(column_headers)):
            column_name = column_headers[j]

            header_cell = HeaderCell(self._head, text=column_name, border_width=self._innerborder_width,
                                     font=header_font, background=header_background, foreground=header_foreground,
                                     padx=padx, pady=pady, border_color=border_color, anchor=header_anchor,
        ...
        if add_scrollbars:
            ...
            self._body = Frame(scrolling_area.innerframe, highlightbackground=border_color, highlightcolor=border_color,
                               highlightthickness=outer_border_width, bd=0)
            self._body.pack()
            ...
            self._body = Frame(self, height=height, highlightbackground=border_color,
                               highlightcolor=border_color, highlightthickness=outer_border_width, bd=0)
        ...
        if column_min_widths is not None:
            for j, min_width in enumerate(column_min_widths):
                if min_width is None:
                    header_cell = self._head.grid_slaves(row=0, column=j)[0]
                    min_width = header_cell.winfo_reqwidth()

                self._body.grid_columnconfigure(j, minsize=min_width)
        else:
            for j in range(len(column_headers)):
                header_cell = self._head.grid_slaves(row=0, column=j)[0]
                min_width = header_cell.winfo_reqwidth()
        ...
        for i in range(number_of_rows, number_of_rows + n):
            ...
            for j in range(number_of_columns):
                ...
                if self._stripped_rows:
                    cell = DataCell(self._body, border_width=self._innerborder_width, variable=var,
                                    border_color=self._bordercolor, padx=self._padx, pady=self._pady,
