...
def get_data_yf():
    ...
    while j < len(companies):
        ...
        table.insert_row([i, companies[j][0], companies[j][1], companies[j][2], companies[j][3], '-'])
        window.update()
...
if __name__ == '__main__':
    ...
    table = Table(master=window, height=600,
                  column_headers=['№', 'Symbol', 'Sector', 'Industry', 'Current/last price in USD'],
                  column_min_widths=[None, 6, 6, 8, None])
    ...
    counter_row += 1
    label_find = Label(master=frame_settings, text='I\'m finding a company which')
    label_find.grid(row=counter_row, column=counter_column)
